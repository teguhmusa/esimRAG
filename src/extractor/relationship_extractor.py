"""
src/indexing/extractor/relationship_extractor.py
-------------------------------------------------
Pure-Python Relationship Extractor for eUICC spec entities.

All relationship dataclasses are defined in .models and imported here.
This module contains only graph-construction logic.
...
def is_primitive(asn1_type: str) -> bool:
    """Return True if the given ASN.1 type name is a built-in primitive."""
...
class RelationshipExtractor:
    """
    Builds all edges in the knowledge graph from entity, section, and
    requirement data.  Returns a flat list of Relationship objects.
    """
    ...
    def _type_section_relationships(
        self,
        type_defs: list[dict],
        sec_index: dict,
    ) -> list[Relationship]:
    ...
    def _section_hierarchy(self, sections: list[dict]) -> list[Relationship]:
        rels = []
        sec_index = {s["section_id"]: s for s in sections}
        for s in sections:
    ...
    def _validation_rule_links(
        ...
        for vr in val_rules:
            ...
            matched_reqs = list(field_to_reqs.get(field_ref, []))
...
def print_relationship_stats(relationships: list[Relationship]) -> None:

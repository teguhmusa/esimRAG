"""
src/indexing/extractor/section_extractor.py
-------------------------------------------
Section Extractor for the eUICC spec.

Builds a hierarchical section tree from parsed PDF pages.
Each section captures: id, title, page range, body text,
ASN.1 blocks, and subsection references.
"""
...
from ..parser.pdf_parser import ParsedPage, TextBlock


# ─────────────────────────────────────────────────────────────────────────────
# Data models (local to this module — tightly coupled with extraction logic)
# ─────────────────────────────────────────────────────────────────────────────

@dataclass
class ASN1Block:
    """A single ASN.1 definition block extracted from the spec PDF."""
...
# Matches section headings like "8.5.2", "12.", or "3.1"
HEADING_RE = re.compile(r"^(\d+(?:\.\d+)*)\s+(.*)")

# ASN.1 type definition opener — e.g. "PE-PUKCodes ::= SEQUENCE {"
ASN1_TYPEDEF_RE = re.compile(
    r"^([A-Za-z][A-Za-z0-9_\-]*)\s*::=\s*"
    r"(SEQUENCE|INTEGER|CHOICE|ENUMERATED|OCTET STRING|UTF8String"
    r"|NULL|BIT STRING|IA5String|BOOLEAN|SET|ANY)"
)
...
class SectionExtractor:
    ...
    Algorithm:
      ...
      4. Close & store section when next heading is found.
    ...
    def __init__(self, skip_toc_pages: int = 6):
        """
        Args:
            skip_toc_pages: Number of pages to skip at the start
                            (cover + TOC).  Default 6 matches this spec.
        """
    ...
    def extract(self, pages: list[ParsedPage]) -> list[Section]:
        ...
        Call build_index() after to get an id → Section mapping.
        """
    ...
    def _parse_asn1_block(
        ...
        Takes the first typedef found when a block contains multiple definitions.
        """
        type_name = None
        asn1_type = None
    ...
    def _build_tree(self, sections: list[Section]) -> None:
        """
        Assign parent_id and populate children lists in-place.
